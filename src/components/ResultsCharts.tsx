...
const ResultsCharts: React.FC<ResultsChartsProps> = ({
	results,
	params,
	chartType = "metrics",
}) => {
	...
	const formatTimeSeriesFor24HourView = (
		...
		for (const point of data) {
			...
			if (formattedData[hour]) {
				...
				formattedData[hour].count = Math.max(
	...
	const processNurseData = () => {
		...
		for (const point of results.nurseUtilizationData) {
			...
			if (formattedData[hour]) {
				formattedData[hour].utilization = Math.max(
					formattedData[hour].utilization,
					utilization
				);
			}
		}
	...
	const pacu1Data = formatTimeSeriesFor24HourView(
		results.pacuPhase1OccupancyData || []
	);
	const pacu2Data = formatTimeSeriesFor24HourView(
		results.pacuPhase2OccupancyData || []
	);
	const wardData = formatTimeSeriesFor24HourView(
		results.wardOccupancyData || []
	);
	const nurseData = processNurseData();

	// Prepare data for OR utilization pie chart
	const orUtilizationData = Object.entries(results.orUtilization || {}).map(
		([key, value]) => ({
			name: key,
	...
	// Render enhanced nurse model metrics if available
	const renderEnhancedNurseMetrics = () => {
		if (
			!params.staffParams?.useEnhancedNurseModel ||
			!results.nurseOvertimeByShift ||
			!results.nurseUtilizationBySkill
		) {
			return null;
		}

		// Prepare data for nurse overtime by shift chart
		const overtimeData = Object.entries(results.nurseOvertimeByShift).map(
			([shiftId, minutes]) => ({
				name: shiftId,
		...
		// Prepare data for nurse utilization by skill chart
		const skillUtilizationData = Object.entries(
			results.nurseUtilizationBySkill
		).map(([skillId, utilization]) => ({
			name: skillId,
			value: utilization * 100, // Convert to percentage
		}));

		// Prepare data for shift coverage chart
		const shiftCoverageData = Object.entries(results.shiftCoverage).map(
			([shiftId, coverage]) => ({
				name: shiftId,
				value: coverage * 100, // Convert to percentage
			})
		);

		// Find the corresponding shift and skill names if available
		...
		return (
			<Card className="mt-6">
				<CardHeader>
					<CardTitle>Hoitajamalli - Lisätiedot</CardTitle>
				</CardHeader>
				<CardContent>
					<Tabs defaultValue="overtime">
						<TabsList>
							<TabsTrigger value="overtime">Ylityöt</TabsTrigger>
							<TabsTrigger value="skills">Taitotasot</TabsTrigger>
							<TabsTrigger value="shifts">Vuorot</TabsTrigger>
						</TabsList>

						<TabsContent value="overtime" className="pt-4">
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div>
									<h4 className="font-medium mb-2">Kokonaisylityötunnit</h4>
									<div className="text-3xl font-bold">
										{results.overtimeHours.toFixed(1)} h
									</div>
									<p className="text-xs text-muted-foreground">
										...
										{new Intl.NumberFormat("fi-FI", {
											style: "currency",
											currency: "EUR",
										}).format(results.costBreakdown.nurseOvertimeCost || 0)}
									</p>
								</div>
								...
								<div className="h-[200px]">
									<ResponsiveContainer width="100%" height="100%">
										...
										>
											...
											<XAxis dataKey="name" tickFormatter={getShiftName} />
											<YAxis
												label={{
													value: "Hours",
													angle: -90,
						...
						<TabsContent value="skills" className="pt-4">
							<div className="h-[300px]">
								<ResponsiveContainer width="100%" height="100%">
									...
									>
										<CartesianGrid strokeDasharray="3 3" />
										<XAxis dataKey="name" tickFormatter={getSkillName} />
										<YAxis
				...
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold">
							{Math.round(results.meanORWaitingTime)} min
						</div>
						<p className="text-xs text-muted-foreground">
							P95: {Math.round(results.p95ORWaitingTime)} min
						</p>
					</CardContent>
				</Card>
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold">
							{Math.round(results.meanPacuTime)} min
						</div>
					</CardContent>
				</Card>
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold">
							{Math.round(results.meanWardTransferDelay)} min
						</div>
						<p className="text-xs text-muted-foreground">
							P95: {Math.round(results.p95WardTransferDelay)} min
						</p>
					</CardContent>
				</Card>
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold">
							{Math.round(results.pacuBlockedTimeRatio * 100)}%
						</div>
						<p className="text-xs text-muted-foreground">
							Potilaat odottavat osastopaikkaa
						</p>
					</CardContent>
				</Card>
			</div>
						...
						<ResponsiveContainer width="100%" height="100%">
							<BarChart
								data={[
									{
										name: "PACU P1",
										value: results.meanPacuPhase1BedOccupancy * 100,
									},
									{
										name: "PACU P2",
										value: results.meanPacuPhase2BedOccupancy * 100,
									},
				...
				<CardContent>
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
						<div>
							...
							<div className="text-3xl font-bold">
								{results.completedSurgeries.length}
							</div>
						</div>
						<div>
							<h4 className="font-medium mb-2">Peruutetut leikkaukset</h4>
							<div className="text-3xl font-bold">
								{results.cancelledSurgeries.length}
							</div>
						</div>
					</div>
							...
							<div>
								...
								<div className="text-3xl font-bold">
									{new Intl.NumberFormat("fi-FI", {
										style: "currency",
										currency: "EUR",
									}).format(results.totalCost || 0)}
								</div>
							</div>
							<div>
								<h4 className="font-medium mb-2">
									Kustannus / suoritettu leikkaus
								</h4>
								<div className="text-3xl font-bold">
									{results.completedSurgeries.length
										? new Intl.NumberFormat("fi-FI", {
								...
								<ResponsiveContainer width="100%" height={300}>
									<BarChart
										data={[
											...
											{
												name: "PACU P1",
												value: results.costBreakdown.pacu1Cost,
											},
											{
												name: "PACU P2",
												value: results.costBreakdown.pacu2Cost,
											},
											{
												name: "Hoitajat",
												value: results.costBreakdown.nurseCost,
											},
											{
												name: "Hoitajat (ylityö)",
												value: results.costBreakdown.nurseOvertimeCost || 0,
											},
											{
												name: "Osastot",
												value: results.costBreakdown.wardCost,
											},
											{
												name: "Peruutukset",
												value: results.costBreakdown.cancellationCost,
											},
										]}
										margin={{ top: 20, right: 30, left: 20, bottom: 5 }}
									>
				...
				<CardContent className="h-[300px]">
					<ResponsiveContainer width="100%" height="100%">
						...
						>
							...
							<XAxis dataKey="label" />
							<YAxis
								label={{ value: "Beds", angle: -90, position: "insideLeft" }}
							/>
